...
function App() {
  ...
  const [queryType, setQueryType] = useState('normal'); // 'normal', 'llm' 或 'ucd'
  ...
  const handleQuery = async () => {
    ...
    try {
      let endpoint = queryType === 'ucd' ? '/api/query_ucd' : 
                     queryType === 'llm' ? '/api/query_llm' : '/api/query';
      let body = {
        question: question,
        mode: queryMode
      };
      ...
      } else if (queryType === 'llm') {
        // LLM 查询结果(大模型生成的答案)
        botMessage = {
          type: 'bot',
          content: data.answer || '生成答案为空',
          mode: data.mode,
          timestamp: new Date().toLocaleTimeString(),
          isUcd: false,
          isLlm: true
    ...
    <div className="App">
      <div className="container">
        ...
        <header className="header">
          <h1><Icon name="database" /> rag + UCD_RAG 系统</h1>
                            ...
                            <div className="message-content">
                              ...
                              {msg.mode && (
                                <div className="message-mode">
                                  模式: {msg.mode} 
                                  {msg.isLlm && <span className="llm-badge"> | 🤖 LLM生成</span>}
                                </div>
                              )}
                        ...
                        <div className="message message-bot">
                          <div className="message-content">
                            <div className="loading">
                              <Icon name="loading" /> 
                              {queryType === 'ucd' ? '正在检索并进行UCD建模...' : 
                               queryType === 'llm' ? '正在检索并生成答案...' : 
                               '正在检索...'}
                            </div>
                          </div>
                        </div>
                    ...
                    <div className="query-controls">
                      <div className="query-mode">
                        ...
                        <select value={queryType} onChange={(e) => setQueryType(e.target.value)}>
                          <option value="normal">普通查询 (仅检索)</option>
                          <option value="llm">LLM查询 (生成答案)</option>
