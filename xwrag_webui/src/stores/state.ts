...
import { checkHealth, xwragStatus } from '@/api/xwrag'
...
const initAuthState = (): { isAuthenticated: boolean; isGuestMode: boolean; coreVersion: string | null; apiVersion: string | null; username: string | null; webuiTitle: string | null; webuiDescription: string | null } => {
  const token = localStorage.getItem('xwrag-API-TOKEN');
  const coreVersion = localStorage.getItem('xwrag-CORE-VERSION');
  const apiVersion = localStorage.getItem('xwrag-API-VERSION');
  const webuiTitle = localStorage.getItem('xwrag-WEBUI-TITLE');
  const webuiDescription = localStorage.getItem('xwrag-WEBUI-DESCRIPTION');
...
export const useAuthStore = create<AuthState>(set => {
  ...
  return {
    ...
    login: (token, isGuest = false, coreVersion = null, apiVersion = null, webuiTitle = null, webuiDescription = null) => {
      localStorage.setItem('xwrag-API-TOKEN', token);

      if (coreVersion) {
        localStorage.setItem('xwrag-CORE-VERSION', coreVersion);
      }
      if (apiVersion) {
        localStorage.setItem('xwrag-API-VERSION', apiVersion);
      }

      if (webuiTitle) {
        localStorage.setItem('xwrag-WEBUI-TITLE', webuiTitle);
      } else {
        localStorage.removeItem('xwrag-WEBUI-TITLE');
      }

      if (webuiDescription) {
        localStorage.setItem('xwrag-WEBUI-DESCRIPTION', webuiDescription);
      } else {
        localStorage.removeItem('xwrag-WEBUI-DESCRIPTION');
      }
    ...
    logout: () => {
      localStorage.removeItem('xwrag-API-TOKEN');
