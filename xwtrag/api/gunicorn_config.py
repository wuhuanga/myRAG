...
import os
import logging
from xwrag.kg.shared_storage import finalize_share_data
from xwrag.utils import setup_logger, get_env_value
...
errorlog = os.getenv("ERROR_LOG", log_file_path)  # Default write to xwrag.log
accesslog = os.getenv("ACCESS_LOG", log_file_path)  # Default write to xwrag.log
...
def on_starting(server):
    ...
    # Log the location of the xwrag log file
    print(f"xwrag log file: {log_file_path}\n")
...
def post_fork(server, worker):
    ...
    setup_logger("xwrag", log_level, add_filter=True, log_file_path=log_file_path)

    # Set up xwrag submodule loggers
    for name in logging.root.manager.loggerDict:
        if name.startswith("xwrag."):
