...
### 1. `test_concurrent_api.py` - 并发测试脚本 ⭐ 推荐

**功能**：真正的并发测试，验证多实例架构和锁机制
...
**输出特点**：
- 线程级别的详细日志（包含线程ID和毫秒级时间戳）
- 并发效率计算（并发加速比）
- 按实例分组的统计信息

### 2. `test_backend_api.py` - 全面测试脚本

**功能**：完整的 API 功能测试，顺序执行所有接口
...
```
实例: satellite_project
├── Thread-1: 上传 卫星工程.docx
└── Thread-2: 上传 子标书01.docx  (同时进行)

预期：两个线程会竞争锁，顺序处理
```

**关键指标**：
- 并发效率应接近 1.0（因为有锁，实际上是串行）
- 不应出现数据竞争错误
...
```
实例1: satellite_project
├── Thread-1: 上传 卫星工程.docx

实例2: tender_docs
├── Thread-2: 上传 子标书01.docx  (并行)
└── Thread-3: 上传 子标书02.docx  (并行)
...
总耗时: 27.98s
平均耗时: 14.00s
并发效率: 1.00x  ← 因为有锁，接近串行

====================================================================================================
                                  测试：不同实例并发上传
====================================================================================================

并发上传 3 个文件到不同实例，线程数: 3

[12:35:30.100] [CrossUpload-0] [INFO] 开始上传: 卫星工程.docx -> satellite_project
[12:35:30.102] [CrossUpload-1] [INFO] 开始上传: 子标书01.docx -> tender_docs
[12:35:30.103] [CrossUpload-2] [INFO] 开始上传: 子标书02.docx -> tender_docs
[12:35:44.567] [CrossUpload-0] [SUCCESS] ✓ 上传成功: 卫星工程.docx (14.47s)
[12:35:46.234] [CrossUpload-1] [SUCCESS] ✓ 上传成功: 子标书01.docx (16.13s)
[12:35:47.890] [CrossUpload-2] [SUCCESS] ✓ 上传成功: 子标书02.docx (17.79s)
...
总耗时: 17.79s
平均耗时: 16.13s
并发效率: 2.72x  ← 真正的并行，效率显著提升

各实例统计:
  satellite_project: ✓ 1 / ✗ 0
  tender_docs: ✓ 2 / ✗ 0
```
