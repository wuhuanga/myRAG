...
from xwrag.utils import TiktokenTokenizer
from xwrag.api.utils_api import get_combined_auth_dependency
from fastapi import Depends


# query mode according to query prefix (bypass is not xwrag quer mode)
...
class OllamaAPI:
    def __init__(self, rag: xwrag, top_k: int = 60, api_key: Optional[str] = None):
        self.rag = rag
    ...
    def setup_routes(self):
        ...
        async def get_tags():
            ...
            return OllamaTagResponse(
                models=[
                    {
                        "name": self.ollama_server_infos.xwrag_MODEL,
                        "model": self.ollama_server_infos.xwrag_MODEL,
                        "modified_at": self.ollama_server_infos.xwrag_CREATED_AT,
                        "size": self.ollama_server_infos.xwrag_SIZE,
                        "digest": self.ollama_server_infos.xwrag_DIGEST,
                        "details": {
                            "parent_model": "",
                            "format": "gguf",
                            "family": self.ollama_server_infos.xwrag_NAME,
                            "families": [self.ollama_server_infos.xwrag_NAME],
                            "parameter_size": "13B",
                ...
                models=[
                    {
                        ...
                        "size_vram": self.ollama_server_infos.xwrag_SIZE,
                    }
                ]
            )
        ...
        async def generate(raw_request: Request):
            ...
            For compatibility purpose, the request is not processed by xwrag,
                    ...
                    async def stream_generator():
                        try:
                            ...
                            if isinstance(response, str):
                                ...
                                data = {
                                    "model": self.ollama_server_infos.xwrag_MODEL,
                                    "created_at": self.ollama_server_infos.xwrag_CREATED_AT,
                                    "response": response,
                                    "done": False,
                                }
        ...
        async def chat(raw_request: Request):
            ...
            Routes user queries through xwrag by selecting query mode based on query prefix.
