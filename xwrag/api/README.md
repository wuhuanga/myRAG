# xwrag Server and WebUI

The xwrag Server is designed to provide a Web UI and API support. The Web UI facilitates document indexing, knowledge graph exploration, and a simple RAG query interface. xwrag Server also provides an Ollama-compatible interface, aiming to emulate xwrag as an Ollama chat model. This allows AI chat bots, such as Open WebUI, to access xwrag easily.
...
### Before Starting xwrag Server

xwrag necessitates the integration of both an LLM (Large Language Model) and an Embedding Model to effectively execute document indexing and querying operations. Prior to the initial deployment of the xwrag server, it is essential to configure the settings for both the LLM and the Embedding Model. xwrag supports binding to various LLM/Embedding backends:
...
It is recommended to use environment variables to configure the xwrag Server. There is an example environment variable file named `env.example` in the root directory of the project. Please copy this file to the startup directory and rename it to `.env`. After that, you can modify the parameters related to the LLM and Embedding models in the `.env` file. It is important to note that the xwrag Server will load the environment variables from `.env` into the system environment variables each time it starts. **xwrag Server will prioritize the settings in the system environment variables to .env file**.
...
### Starting xwrag Server

The xwrag Server supports two operational modes:
...
```
xwrag-gunicorn --workers 4
```

When starting xwrag, the current working directory must contain the `.env` configuration file. **It is intentionally designed that the `.env` file must be placed in the startup directory**. The purpose of this is to allow users to launch multiple xwrag instances simultaneously and configure different `.env` files for different instances. **After modifying the `.env` file, you need to reopen the terminal for the new settings to take effect.** This is because each time xwrag Server starts, it loads the environment variables from the `.env` file into the system environment variables, and system environment variables have higher precedence.
...
- `--workspace`: Workspace name, used to logically isolate data between multiple xwrag instances (default: empty)

### Launching xwrag Server with Docker
...
services:
  xwrag:
    container_name: xwrag
    image: ghcr.io/hkuds/xwrag:latest
    ports:
      - "${PORT:-9621}:9621"
    volumes:
...
* Start the xwrag Server with the following command:
...
> You can get the official docker compose file from here: [docker-compose.yml](https://raw.githubusercontent.com/HKUDS/xwrag/refs/heads/main/docker-compose.yml). For historical versions of xwrag docker images, visit this link: [xwrag Docker Images](https://github.com/HKUDS/xwrag/pkgs/container/xwrag)

### Starting Multiple xwrag Instances

There are two ways to start multiple xwrag instances. The first way is to configure a completely independent working environment for each instance. This requires creating a separate working directory for each instance and placing a dedicated `.env` configuration file in that directory. The server listening ports in the configuration files of different instances cannot be the same. Then, you can start the service by running `xwrag-server` in the working directory.

The second way is for all instances to share the same set of `.env` configuration files, and then use command-line arguments to specify different server listening ports and workspaces for each instance. You can start multiple xwrag instances in the same working directory with different command-line arguments. For example:

```
# Start instance 1
xwrag-server --port 9621 --workspace space1

# Start instance 2
xwrag-server --port 9622 --workspace space2
```
...
The xwrag Server can operate in the `Gunicorn + Uvicorn` preload mode. Gunicorn's multiple worker (multiprocess) capability prevents document indexing tasks from blocking RAG queries. Using CPU-exhaustive document extraction tools, such as docling, can lead to the entire system being blocked in pure Uvicorn mode.

Though xwrag Server uses one worker to process the document indexing pipeline, with the async task support of Uvicorn, multiple files can be processed in parallel. The bottleneck of document indexing speed mainly lies with the LLM. If your LLM supports high concurrency, you can accelerate document indexing by increasing the concurrency level of the LLM. Below are several environment variables related to concurrent processing, along with their default values:
...
### Install xwrag as a Linux Service

Create your service file `xwrag.service` from the sample file: `xwrag.service.example`. Modify the `WorkingDirectory` and `ExecStart` in the service file:

```text
Description=xwrag Ollama Service
WorkingDirectory=<xwrag installed directory>
ExecStart=<xwrag installed directory>/xwrag/api/xwrag-api
```

Modify your service startup script: `xwrag-api`. Change your Python virtual environment activation command as needed:

```shell
#!/bin/bash

# your python virtual environment activation
source /home/netman/xwrag-xyj/venv/bin/activate
# start xwrag api server
xwrag-server
```

Install xwrag service. If your system is Ubuntu, the following commands will work:

```shell
sudo cp xwrag.service /etc/systemd/system/
sudo systemctl daemon-reload
sudo systemctl start xwrag.service
sudo systemctl status xwrag.service
sudo systemctl enable xwrag.service
```

## Ollama Emulation

We provide Ollama-compatible interfaces for xwrag, aiming to emulate xwrag as an Ollama chat model. This allows AI chat frontends supporting Ollama, such as Open WebUI, to access xwrag easily.

### Connect Open WebUI to xwrag

After starting the xwrag-server, you can add an Ollama-type connection in the Open WebUI admin panel. And then a model named `xwrag:latest` will appear in Open WebUI's model management interface. Users can then send queries to xwrag through the chat interface. You should install xwrag as a service for this use case.
...
The default query mode is `hybrid` if you send a message (query) from the Ollama interface of xwrag. You can select query mode by sending a message with a query prefix.

A query prefix in the query string can determine which xwrag query mode is used to generate the response for the query. The supported prefixes include:
...
For example, the chat message `/mix What's xwrag?` will trigger a mix mode query for xwrag. A chat message without a query prefix will trigger a hybrid mode query by default.

`/bypass` is not a xwrag query mode; it will tell the API Server to pass the query directly to the underlying LLM, including the chat history. So the user can use the LLM to answer questions based on the chat history. If you are using Open WebUI as a front end, you can just switch the model to a normal LLM instead of using the `/bypass` prefix.

`/context` is also not a xwrag query mode; it will tell xwrag to return only the context information prepared for the LLM. You can check the context if it's what you want, or process the context by yourself.

### Add user prompt in chat

When using xwrag for content queries, avoid combining the search process with unrelated output processing, as this significantly impacts query effectiveness. User prompt is specifically designed to address this issue — it does not participate in the RAG retrieval phase, but rather guides the LLM on how to process the retrieved results after the query is completed. We can append square brackets to the query prefix to provide the LLM with the user prompt:
...
By default, the xwrag Server can be accessed without any authentication. We can configure the server with an API Key or account credentials to secure it.
...
The API key is passed using the request header `X-API-Key`. Below is an example of accessing the xwrag Server via API:
...
xwrag API Server implements JWT-based authentication using the HS256 algorithm. To enable secure access control, the following environment variables are required:
...
RESOURCE_GROUP_NAME=xwrag
LOCATION=swedencentral
RESOURCE_NAME=xwrag-OpenAI
...
## xwrag Server Configuration in Detail
...
xwrag supports binding to various LLM/Embedding backends:
...
```
xwrag-server --llm-binding openai --help
xwrag-server --llm-binding ollama --help
xwrag-server --embedding-binding ollama --help
```
...
xwrag Server offers various storage implementations, with the default being an in-memory database that persists data to the WORKING_DIR directory. Additionally, xwrag supports a wide range of storage solutions including PostgreSQL, MongoDB, FAISS, Milvus, Qdrant, Neo4j, Memgraph, and Redis. For detailed information on supported storage options, please refer to the storage section in the README.md file located in the root directory.
...
You cannot change storage implementation selection after adding documents to xwrag. Data migration from one storage implementation to another is not supported yet. For further information, please read the sample env file or config.ini file.

### xwrag API Server Command Line Options
...
| --key                 | None          | API key for authentication. Protects the xwrag server against unauthorized access                                            |
...
Reranking query-recalled chunks can significantly enhance retrieval quality by re-ordering documents based on an optimized relevance scoring model. xwrag currently supports the following rerank providers:
...
### For JWT Auth
# AUTH_ACCOUNTS='admin:admin123,user1:pass456'
# TOKEN_SECRET=your-key-for-xwrag-API-Server-xxx
# TOKEN_EXPIRE_HOURS=48

# xwrag_API_KEY=your-secure-api-key-here-123
# WHITELIST_PATHS=/api/*
# WHITELIST_PATHS=/health,/api/*
...
xwrag implements asynchronous document indexing to enable frontend monitoring and querying of document processing progress. Upon uploading files or inserting text through designated endpoints, a unique Track ID is returned to facilitate real-time progress monitoring.
