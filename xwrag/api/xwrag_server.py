...
from xwrag.api.utils_api import (
...
from xwrag.utils import get_env_value
from xwrag import xwrag, __version__ as core_version
from xwrag.api import __api_version__
from xwrag.types import GPTKeywordExtractionFormat
from xwrag.utils import EmbeddingFunc
from xwrag.constants import (
...
from xwrag.api.routers.document_routes import (
    DocumentManager,
    create_document_routes,
)
from xwrag.api.routers.query_routes import create_query_routes
from xwrag.api.routers.graph_routes import create_graph_routes
from xwrag.api.routers.ollama_api import OllamaAPI

from xwrag.utils import logger, set_verbose_debug
...
from xwrag.api.auth import auth_handler
...
class LLMConfigCache:
    ...
    def __init__(self, args):
        ...
        if args.llm_binding in ["openai", "azure_openai"]:
            from xwrag.llm.binding_options import OpenAILLMOptions
        ...
        if args.llm_binding == "ollama":
            try:
                from xwrag.llm.binding_options import OllamaLLMOptions
        ...
        if args.embedding_binding == "ollama":
            try:
                from xwrag.llm.binding_options import OllamaEmbeddingOptions
...
def create_app(args):
    ...
    api_key = os.getenv("xwrag_API_KEY") or args.key
    ...
    # Initialize FastAPI
    app_kwargs = {
        "title": "xwrag Server API",
        "description": (
            "Providing API for xwrag core, Web UI and Ollama Model Emulation"
            + "(With authentication)"
            if api_key
            else ""
        ),
    ...
    def create_optimized_openai_llm_func(
        ...
        async def optimized_openai_alike_model_complete(
            ...
            **kwargs,
            ...
            from xwrag.llm.openai import openai_complete_if_cache
    ...
    def create_optimized_azure_openai_llm_func(
        ...
        async def optimized_azure_openai_model_complete(
            ...
            **kwargs,
            ...
            from xwrag.llm.azure_openai import azure_openai_complete_if_cache
    ...
    def create_llm_model_func(binding: str):
        ...
        try:
            if binding == "lollms":
                from xwrag.llm.lollms import lollms_model_complete

                return lollms_model_complete
            elif binding == "ollama":
                from xwrag.llm.ollama import ollama_model_complete
    ...
    def create_optimized_embedding_function(
        ...
        async def optimized_embedding_function(texts):
            try:
                if binding == "lollms":
                    from xwrag.llm.lollms import lollms_embed

                    return await lollms_embed(
                        texts, embed_model=model, host=host, api_key=api_key
                    )
                elif binding == "ollama":
                    from xwrag.llm.ollama import ollama_embed
                    ...
                    from xwrag.llm.azure_openai import azure_openai_embed

                    return await azure_openai_embed(texts, model=model, api_key=api_key)
                elif binding == "aws_bedrock":
                    from xwrag.llm.bedrock import bedrock_embed
    ...
    async def bedrock_model_complete(
        ...
        **kwargs,
        ...
        # Lazy import
        from xwrag.llm.bedrock import bedrock_complete_if_cache
    ...
    if args.rerank_binding != "null":
        from xwrag.rerank import cohere_rerank, jina_rerank, ali_rerank
    ...
    from xwrag.api.config import OllamaServerInfos
    ...
    try:
        ...
        logger.error(f"Failed to initialize xwrag: {e}")
        raise
...
def configure_logging():
    ...
    for logger_name in ["uvicorn", "uvicorn.access", "uvicorn.error", "xwrag"]:
