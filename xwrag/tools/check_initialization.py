#!/usr/bin/env python3
"""
Diagnostic tool to check xwrag initialization status.

This tool helps developers verify that their xwrag instance is properly
initialized before use, preventing common initialization errors.

Usage:
    python -m xwrag.tools.check_initialization
"""
...
from xwrag.base import StoragesStatus


async def check_xwrag_setup(rag_instance: xwrag, verbose: bool = False) -> bool:
    """
    Check if a xwrag instance is properly initialized.

    Args:
        rag_instance: The xwrag instance to check
    ...
    print("🔍 Checking xwrag initialization status...\n")

    # Check storage initialization status
    if not hasattr(rag_instance, "_storages_status"):
        issues.append("xwrag instance missing _storages_status attribute")
    ...
    try:
        from xwrag.kg.shared_storage import get_namespace_data
    ...
    if issues:
        ...
        print("  from xwrag.kg.shared_storage import initialize_pipeline_status")
        print("  await initialize_pipeline_status()")
        print(
            "\n📚 Documentation: https://github.com/HKUDS/xwrag#important-initialization-requirements"
        )
        ...
        print("✅ xwrag is properly initialized and ready to use!")
...
async def demo():
    ...
    from xwrag.llm.openai import openai_embed, gpt_4o_mini_complete
    from xwrag.kg.shared_storage import initialize_pipeline_status

    print("=" * 50)
    print("xwrag Initialization Diagnostic Tool")
    print("=" * 50)
    ...
    await check_xwrag_setup(rag, verbose=True)
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Check xwrag initialization status")
    parser.add_argument(
    ...
    if args.demo:
        ...
        print("Or import this module and use check_xwrag_setup() with your instance")
