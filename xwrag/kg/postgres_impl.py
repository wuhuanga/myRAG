...
from xwrag.types import KnowledgeGraph, KnowledgeGraphNode, KnowledgeGraphEdge
...
# allows to use different .env file for each xwrag instance
...
class PostgreSQLDB:
    ...
    async def _migrate_llm_cache_schema(self):
        ...
        try:
            ...
            WHERE table_name = 'xwrag_llm_cache'
            ...
            if "chunk_id" not in existing_column_names:
                logger.info("Adding chunk_id column to xwrag_LLM_CACHE table")
                add_chunk_id_sql = """
                ALTER TABLE xwrag_LLM_CACHE
                ADD COLUMN chunk_id VARCHAR(255) NULL
                """
                await self.execute(add_chunk_id_sql)
                logger.info(
                    "Successfully added chunk_id column to xwrag_LLM_CACHE table"
                )
            else:
                logger.info(
                    "chunk_id column already exists in xwrag_LLM_CACHE table"
                )

            # Add missing cache_type column
            if "cache_type" not in existing_column_names:
                logger.info("Adding cache_type column to xwrag_LLM_CACHE table")
                add_cache_type_sql = """
                ALTER TABLE xwrag_LLM_CACHE
                ...
                logger.info(
                    "Successfully added cache_type column to xwrag_LLM_CACHE table"
                )
                ...
                logger.info(
                    "cache_type column already exists in xwrag_LLM_CACHE table"
                )

            # Add missing queryparam column
            if "queryparam" not in existing_column_names:
                logger.info("Adding queryparam column to xwrag_LLM_CACHE table")
                add_queryparam_sql = """
                ALTER TABLE xwrag_LLM_CACHE
                ...
                logger.info(
                    "Successfully added queryparam column to xwrag_LLM_CACHE table"
                )
            else:
                logger.info(
                    "queryparam column already exists in xwrag_LLM_CACHE table"
                )

            # Remove deprecated mode field if it exists
            if "mode" in existing_column_names:
                logger.info(
                    "Removing deprecated mode column from xwrag_LLM_CACHE table"
                )

                # First, drop the primary key constraint that includes mode
                drop_pk_sql = """
                ALTER TABLE xwrag_LLM_CACHE
                DROP CONSTRAINT IF EXISTS xwrag_LLM_CACHE_PK
                """
                ...
                logger.info(
                    "Successfully removed mode column from xwrag_LLM_CACHE table"
                )

                # Create new primary key constraint without mode
                add_pk_sql = """
                ALTER TABLE xwrag_LLM_CACHE
                ADD CONSTRAINT xwrag_LLM_CACHE_PK PRIMARY KEY (workspace, id)
                """
                await self.execute(add_pk_sql)
                logger.info("Created new primary key constraint (workspace, id)")
            else:
                logger.info("mode column does not exist in xwrag_LLM_CACHE table")
    ...
    async def _migrate_timestamp_columns(self):
        ...
        tables_to_migrate = {
            "xwrag_VDB_ENTITY": ["create_time", "update_time"],
            "xwrag_VDB_RELATION": ["create_time", "update_time"],
            "xwrag_DOC_CHUNKS": ["create_time", "update_time"],
            "xwrag_DOC_STATUS": ["created_at", "updated_at"],
        }
    ...
    async def _migrate_doc_chunks_to_vdb_chunks(self):
        """
        Migrate data from xwrag_DOC_CHUNKS to xwrag_VDB_CHUNKS if specific conditions are met.
        This migration is intended for users who are upgrading and have an older table structure
        where xwrag_DOC_CHUNKS contained a `content_vector` column.

        """
        try:
            # 1. Check if the new table xwrag_VDB_CHUNKS is empty
            vdb_chunks_count_sql = "SELECT COUNT(1) as count FROM xwrag_VDB_CHUNKS"
            ...
            if vdb_chunks_count_result and vdb_chunks_count_result["count"] > 0:
                logger.info(
                    "Skipping migration: xwrag_VDB_CHUNKS already contains data."
                )
                return
            ...
            WHERE table_name = 'xwrag_doc_chunks' AND column_name = 'content_vector'
            """
            column_exists = await self.query(check_column_sql)
            if not column_exists:
                logger.info(
                    "Skipping migration: `content_vector` not found in xwrag_DOC_CHUNKS"
                )
                return

            # 3. Check if the old table xwrag_DOC_CHUNKS has data
            doc_chunks_count_sql = "SELECT COUNT(1) as count FROM xwrag_DOC_CHUNKS"
            ...
            if not doc_chunks_count_result or doc_chunks_count_result["count"] == 0:
                logger.info("Skipping migration: xwrag_DOC_CHUNKS is empty.")
                return

            # 4. Perform the migration
            logger.info(
                "Starting data migration from xwrag_DOC_CHUNKS to xwrag_VDB_CHUNKS..."
            )
            migration_sql = """
            INSERT INTO xwrag_VDB_CHUNKS (
            ...
            SELECT
                id, workspace, full_doc_id, chunk_order_index, tokens, content,
                content_vector, file_path, create_time, update_time
            FROM xwrag_DOC_CHUNKS
            ON CONFLICT (workspace, id) DO NOTHING;
            """
            await self.execute(migration_sql)
            logger.info("Data migration to xwrag_VDB_CHUNKS completed successfully.")

        except Exception as e:
            logger.error(f"Failed during data migration to xwrag_VDB_CHUNKS: {e}")
    ...
    async def _migrate_llm_cache_to_flattened_keys(self):
        ...
        try:
            ...
            check_sql = """
            SELECT COUNT(*) as count FROM xwrag_LLM_CACHE
            WHERE id NOT LIKE '%:%'
            """
            result = await self.query(check_sql)
            ...
            WITH new_ids AS (
                ...
                FROM xwrag_LLM_CACHE
                WHERE id NOT LIKE '%:%'
            )
            SELECT COUNT(*) as conflicts
            FROM new_ids n1
            JOIN xwrag_LLM_CACHE existing
            ...
            verify_sql = """
            SELECT COUNT(*) as remaining_old FROM xwrag_LLM_CACHE
            WHERE id NOT LIKE '%:%'
            """
    ...
    async def _migrate_doc_status_add_chunks_list(self):
        """Add chunks_list column to xwrag_DOC_STATUS table if it doesn't exist"""
        try:
            # Check if chunks_list column exists
            check_column_sql = """
            SELECT column_name
            FROM information_schema.columns
            WHERE table_name = 'xwrag_doc_status'
            AND column_name = 'chunks_list'
            """

            column_info = await self.query(check_column_sql)
            if not column_info:
                logger.info("Adding chunks_list column to xwrag_DOC_STATUS table")
                add_column_sql = """
                ALTER TABLE xwrag_DOC_STATUS
                ADD COLUMN chunks_list JSONB NULL DEFAULT '[]'::jsonb
                """
                await self.execute(add_column_sql)
                logger.info(
                    "Successfully added chunks_list column to xwrag_DOC_STATUS table"
                )
            else:
                logger.info(
                    "chunks_list column already exists in xwrag_DOC_STATUS table"
                )
        except Exception as e:
            logger.warning(
                f"Failed to add chunks_list column to xwrag_DOC_STATUS: {e}"
            )

    async def _migrate_text_chunks_add_llm_cache_list(self):
        """Add llm_cache_list column to xwrag_DOC_CHUNKS table if it doesn't exist"""
        try:
            # Check if llm_cache_list column exists
            check_column_sql = """
            SELECT column_name
            FROM information_schema.columns
            WHERE table_name = 'xwrag_doc_chunks'
            AND column_name = 'llm_cache_list'
            """

            column_info = await self.query(check_column_sql)
            if not column_info:
                logger.info("Adding llm_cache_list column to xwrag_DOC_CHUNKS table")
                add_column_sql = """
                ALTER TABLE xwrag_DOC_CHUNKS
                ADD COLUMN llm_cache_list JSONB NULL DEFAULT '[]'::jsonb
                """
                await self.execute(add_column_sql)
                logger.info(
                    "Successfully added llm_cache_list column to xwrag_DOC_CHUNKS table"
                )
            else:
                logger.info(
                    "llm_cache_list column already exists in xwrag_DOC_CHUNKS table"
                )
        except Exception as e:
            logger.warning(
                f"Failed to add llm_cache_list column to xwrag_DOC_CHUNKS: {e}"
            )

    async def _migrate_doc_status_add_track_id(self):
        """Add track_id column to xwrag_DOC_STATUS table if it doesn't exist and create index"""
        try:
            ...
            column_info = await self.query(check_column_sql)
            if not column_info:
                logger.info("Adding track_id column to xwrag_DOC_STATUS table")
                add_column_sql = """
                ALTER TABLE xwrag_DOC_STATUS
                ...
                await self.execute(add_column_sql)
                logger.info(
                    "Successfully added track_id column to xwrag_DOC_STATUS table"
                )
            else:
                logger.info(
                    "track_id column already exists in xwrag_DOC_STATUS table"
                )
            ...
            SELECT indexname
            FROM pg_indexes
            WHERE tablename = 'xwrag_doc_status'
            AND indexname = 'idx_xwrag_doc_status_track_id'
            """

            index_info = await self.query(check_index_sql)
            if not index_info:
                logger.info(
                    "Creating index on track_id column for xwrag_DOC_STATUS table"
                )
                create_index_sql = """
                CREATE INDEX idx_xwrag_doc_status_track_id ON xwrag_DOC_STATUS (track_id)
                """
                await self.execute(create_index_sql)
                logger.info(
                    "Successfully created index on track_id column for xwrag_DOC_STATUS table"
                )
            else:
                logger.info(
                    "Index on track_id column already exists for xwrag_DOC_STATUS table"
                )

        except Exception as e:
            logger.warning(
                f"Failed to add track_id column or index to xwrag_DOC_STATUS: {e}"
            )

    async def _migrate_doc_status_add_metadata_error_msg(self):
        """Add metadata and error_msg columns to xwrag_DOC_STATUS table if they don't exist"""
        try:
            ...
            if not metadata_info:
                logger.info("Adding metadata column to xwrag_DOC_STATUS table")
                add_metadata_sql = """
                ALTER TABLE xwrag_DOC_STATUS
                ...
                logger.info(
                    "Successfully added metadata column to xwrag_DOC_STATUS table"
                )
            else:
                logger.info(
                    "metadata column already exists in xwrag_DOC_STATUS table"
                )
            ...
            if not error_msg_info:
                logger.info("Adding error_msg column to xwrag_DOC_STATUS table")
                add_error_msg_sql = """
                ALTER TABLE xwrag_DOC_STATUS
                ...
                logger.info(
                    "Successfully added error_msg column to xwrag_DOC_STATUS table"
                )
            else:
                logger.info(
                    "error_msg column already exists in xwrag_DOC_STATUS table"
                )

        except Exception as e:
            logger.warning(
                f"Failed to add metadata/error_msg columns to xwrag_DOC_STATUS: {e}"
            )
    ...
    async def check_tables(self):
        ...
        # Create pagination optimization indexes for xwrag_DOC_STATUS
        try:
            await self._create_pagination_indexes()
        except Exception as e:
            logger.error(f"PostgreSQL, Failed to create pagination indexes: {e}")

        # Migrate to ensure new tables xwrag_FULL_ENTITIES and xwrag_FULL_RELATIONS exist
        try:
    ...
    async def _migrate_create_full_entities_relations_tables(self):
        """Create xwrag_FULL_ENTITIES and xwrag_FULL_RELATIONS tables if they don't exist"""
        tables_to_check = [
            {
                "name": "xwrag_FULL_ENTITIES",
                "ddl": TABLES["xwrag_FULL_ENTITIES"]["ddl"],
                "description": "Full entities storage table",
            },
            {
                "name": "xwrag_FULL_RELATIONS",
                "ddl": TABLES["xwrag_FULL_RELATIONS"]["ddl"],
    ...
    async def _create_pagination_indexes(self):
        """Create indexes to optimize pagination queries for xwrag_DOC_STATUS"""
        indexes = [
            {
                "name": "idx_xwrag_doc_status_workspace_status_updated_at",
                "sql": "CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_xwrag_doc_status_workspace_status_updated_at ON xwrag_DOC_STATUS (workspace, status, updated_at DESC)",
                "description": "Composite index for workspace + status + updated_at pagination",
            },
            {
                "name": "idx_xwrag_doc_status_workspace_status_created_at",
                "sql": "CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_xwrag_doc_status_workspace_status_created_at ON xwrag_DOC_STATUS (workspace, status, created_at DESC)",
                "description": "Composite index for workspace + status + created_at pagination",
            },
            {
                "name": "idx_xwrag_doc_status_workspace_updated_at",
                "sql": "CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_xwrag_doc_status_workspace_updated_at ON xwrag_DOC_STATUS (workspace, updated_at DESC)",
                "description": "Index for workspace + updated_at pagination (all statuses)",
            },
            {
                "name": "idx_xwrag_doc_status_workspace_created_at",
                "sql": "CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_xwrag_doc_status_workspace_created_at ON xwrag_DOC_STATUS (workspace, created_at DESC)",
                "description": "Index for workspace + created_at pagination (all statuses)",
            },
            {
                "name": "idx_xwrag_doc_status_workspace_id",
                "sql": "CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_xwrag_doc_status_workspace_id ON xwrag_DOC_STATUS (workspace, id)",
                "description": "Index for workspace + id sorting",
            },
            {
                "name": "idx_xwrag_doc_status_workspace_file_path",
                "sql": "CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_xwrag_doc_status_workspace_file_path ON xwrag_DOC_STATUS (workspace, file_path)",
...
class PGVectorStorage(BaseVectorStorage):
    ...
    async def delete_entity(self, entity_name: str) -> None:
        ...
        try:
            ...
            delete_sql = """DELETE FROM xwrag_VDB_ENTITY
    ...
    async def delete_entity_relation(self, entity_name: str) -> None:
        ...
        try:
            ...
            delete_sql = """DELETE FROM xwrag_VDB_RELATION
...
class PGDocStatusStorage(DocStatusStorage):
    ...
    async def get_by_id(self, id: str) -> Union[dict[str, Any], None]:
        sql = "select * from xwrag_DOC_STATUS where workspace=$1 and id=$2"
        params = {"workspace": self.workspace, "id": id}
    ...
    async def get_by_ids(self, ids: list[str]) -> list[dict[str, Any]]:
        ...
        sql = "SELECT * FROM xwrag_DOC_STATUS WHERE workspace=$1 AND id = ANY($2)"
    ...
    async def get_doc_by_file_path(self, file_path: str) -> Union[dict[str, Any], None]:
        ...
        """
        sql = "select * from xwrag_DOC_STATUS where workspace=$1 and file_path=$2"
    ...
    async def get_docs_by_status(
        ...
        sql = "select * from xwrag_DOC_STATUS where workspace=$1 and status=$2"
    ...
    async def get_docs_by_track_id(
        ...
        sql = "select * from xwrag_DOC_STATUS where workspace=$1 and track_id=$2"
    ...
    async def get_docs_paginated(
        ...
        count_sql = f"SELECT COUNT(*) as total FROM xwrag_DOC_STATUS {where_clause}"
        count_result = await self.db.query(count_sql, list(params.values()))
        total_count = count_result["total"] if count_result else 0

        # Query for paginated data
        data_sql = f"""
            SELECT * FROM xwrag_DOC_STATUS
            {where_clause}
            {order_clause}
    ...
    async def upsert(self, data: dict[str, dict[str, Any]]) -> None:
        ...
        sql = """insert into xwrag_DOC_STATUS(workspace,id,content_summary,content_length,chunks_count,status,file_path,chunks_list,track_id,metadata,error_msg,created_at,updated_at)
...
class PGGraphStorage(BaseGraphStorage):
    ...
    async def upsert_edge(
        ...
        query = """SELECT * FROM cypher('%s', $$
            ...
            tgt_label,
            edge_properties,
            edge_properties,  # https://github.com/HKUDS/xwrag/issues/1438#issuecomment-2826000195
        )
...
NAMESPACE_TABLE_MAP = {
    NameSpace.KV_STORE_FULL_DOCS: "xwrag_DOC_FULL",
    NameSpace.KV_STORE_TEXT_CHUNKS: "xwrag_DOC_CHUNKS",
    NameSpace.KV_STORE_FULL_ENTITIES: "xwrag_FULL_ENTITIES",
    NameSpace.KV_STORE_FULL_RELATIONS: "xwrag_FULL_RELATIONS",
    NameSpace.KV_STORE_LLM_RESPONSE_CACHE: "xwrag_LLM_CACHE",
    NameSpace.VECTOR_STORE_CHUNKS: "xwrag_VDB_CHUNKS",
    NameSpace.VECTOR_STORE_ENTITIES: "xwrag_VDB_ENTITY",
    NameSpace.VECTOR_STORE_RELATIONSHIPS: "xwrag_VDB_RELATION",
    NameSpace.DOC_STATUS: "xwrag_DOC_STATUS",
}
...
TABLES = {
    "xwrag_DOC_FULL": {
        "ddl": """CREATE TABLE xwrag_DOC_FULL (
                    id VARCHAR(255),
	                ...
	                CONSTRAINT xwrag_DOC_FULL_PK PRIMARY KEY (workspace, id)
                    )"""
    },
    "xwrag_DOC_CHUNKS": {
        "ddl": """CREATE TABLE xwrag_DOC_CHUNKS (
                    id VARCHAR(255),
                    workspace VARCHAR(255),
                    ...
                    create_time TIMESTAMP(0) DEFAULT CURRENT_TIMESTAMP,
                    update_time TIMESTAMP(0) DEFAULT CURRENT_TIMESTAMP,
	                CONSTRAINT xwrag_DOC_CHUNKS_PK PRIMARY KEY (workspace, id)
                    )"""
    },
    "xwrag_VDB_CHUNKS": {
        "ddl": f"""CREATE TABLE xwrag_VDB_CHUNKS (
                    id VARCHAR(255),
                    workspace VARCHAR(255),
                    full_doc_id VARCHAR(256),
                    chunk_order_index INTEGER,
                    ...
                    create_time TIMESTAMP(0) DEFAULT CURRENT_TIMESTAMP,
                    update_time TIMESTAMP(0) DEFAULT CURRENT_TIMESTAMP,
	                CONSTRAINT xwrag_VDB_CHUNKS_PK PRIMARY KEY (workspace, id)
                    )"""
    },
    "xwrag_VDB_ENTITY": {
        "ddl": f"""CREATE TABLE xwrag_VDB_ENTITY (
                    id VARCHAR(255),
                    workspace VARCHAR(255),
                    ...
                    file_path TEXT NULL,
	                CONSTRAINT xwrag_VDB_ENTITY_PK PRIMARY KEY (workspace, id)
                    )"""
    },
    "xwrag_VDB_RELATION": {
        "ddl": f"""CREATE TABLE xwrag_VDB_RELATION (
                    id VARCHAR(255),
                    workspace VARCHAR(255),
                    ...
                    chunk_ids VARCHAR(255)[] NULL,
                    file_path TEXT NULL,
	                CONSTRAINT xwrag_VDB_RELATION_PK PRIMARY KEY (workspace, id)
                    )"""
    },
    "xwrag_LLM_CACHE": {
        "ddl": """CREATE TABLE xwrag_LLM_CACHE (
	                ...
	                CONSTRAINT xwrag_LLM_CACHE_PK PRIMARY KEY (workspace, id)
                    )"""
    },
    "xwrag_DOC_STATUS": {
        "ddl": """CREATE TABLE xwrag_DOC_STATUS (
	               workspace varchar(255) NOT NULL,
	               id varchar(255) NOT NULL,
	               ...
	               CONSTRAINT xwrag_DOC_STATUS_PK PRIMARY KEY (workspace, id)
	              )"""
    },
    "xwrag_FULL_ENTITIES": {
        "ddl": """CREATE TABLE xwrag_FULL_ENTITIES (
                    id VARCHAR(255),
                    workspace VARCHAR(255),
                    entity_names JSONB,
                    count INTEGER,
                    create_time TIMESTAMP(0) DEFAULT CURRENT_TIMESTAMP,
                    update_time TIMESTAMP(0) DEFAULT CURRENT_TIMESTAMP,
                    CONSTRAINT xwrag_FULL_ENTITIES_PK PRIMARY KEY (workspace, id)
                    )"""
    },
    "xwrag_FULL_RELATIONS": {
        "ddl": """CREATE TABLE xwrag_FULL_RELATIONS (
                    id VARCHAR(255),
                    workspace VARCHAR(255),
                    ...
                    create_time TIMESTAMP(0) DEFAULT CURRENT_TIMESTAMP,
                    update_time TIMESTAMP(0) DEFAULT CURRENT_TIMESTAMP,
                    CONSTRAINT xwrag_FULL_RELATIONS_PK PRIMARY KEY (workspace, id)
                    )"""
    },
}
...
SQL_TEMPLATES = {
    ...
    "get_by_id_full_docs": """SELECT id, COALESCE(content, '') as content
                                FROM xwrag_DOC_FULL WHERE workspace=$1 AND id=$2
                            """,
    "get_by_id_text_chunks": """SELECT id, tokens, COALESCE(content, '') as content,
                                ...
                                FROM xwrag_DOC_CHUNKS WHERE workspace=$1 AND id=$2
                            """,
    "get_by_id_llm_response_cache": """SELECT id, original_prompt, return_value, chunk_id, cache_type, queryparam,
                                EXTRACT(EPOCH FROM create_time)::BIGINT as create_time,
                                EXTRACT(EPOCH FROM update_time)::BIGINT as update_time
                                FROM xwrag_LLM_CACHE WHERE workspace=$1 AND id=$2
                               """,
    "get_by_ids_full_docs": """SELECT id, COALESCE(content, '') as content
                                 FROM xwrag_DOC_FULL WHERE workspace=$1 AND id IN ({ids})
                            """,
    "get_by_ids_text_chunks": """SELECT id, tokens, COALESCE(content, '') as content,
                                  ...
                                  EXTRACT(EPOCH FROM create_time)::BIGINT as create_time,
                                  EXTRACT(EPOCH FROM update_time)::BIGINT as update_time
                                   FROM xwrag_DOC_CHUNKS WHERE workspace=$1 AND id IN ({ids})
                                """,
    "get_by_ids_llm_response_cache": """SELECT id, original_prompt, return_value, chunk_id, cache_type, queryparam,
                                 EXTRACT(EPOCH FROM create_time)::BIGINT as create_time,
                                 EXTRACT(EPOCH FROM update_time)::BIGINT as update_time
                                 FROM xwrag_LLM_CACHE WHERE workspace=$1 AND id IN ({ids})
                                """,
    "get_by_id_full_entities": """SELECT id, entity_names, count,
                                EXTRACT(EPOCH FROM create_time)::BIGINT as create_time,
                                EXTRACT(EPOCH FROM update_time)::BIGINT as update_time
                                FROM xwrag_FULL_ENTITIES WHERE workspace=$1 AND id=$2
                               """,
    "get_by_id_full_relations": """SELECT id, relation_pairs, count,
                                EXTRACT(EPOCH FROM create_time)::BIGINT as create_time,
                                EXTRACT(EPOCH FROM update_time)::BIGINT as update_time
                                FROM xwrag_FULL_RELATIONS WHERE workspace=$1 AND id=$2
                               """,
    "get_by_ids_full_entities": """SELECT id, entity_names, count,
                                 EXTRACT(EPOCH FROM create_time)::BIGINT as create_time,
                                 EXTRACT(EPOCH FROM update_time)::BIGINT as update_time
                                 FROM xwrag_FULL_ENTITIES WHERE workspace=$1 AND id IN ({ids})
                                """,
    "get_by_ids_full_relations": """SELECT id, relation_pairs, count,
                                 EXTRACT(EPOCH FROM create_time)::BIGINT as create_time,
                                 EXTRACT(EPOCH FROM update_time)::BIGINT as update_time
                                 FROM xwrag_FULL_RELATIONS WHERE workspace=$1 AND id IN ({ids})
                                """,
    "filter_keys": "SELECT id FROM {table_name} WHERE workspace=$1 AND id IN ({ids})",
    "upsert_doc_full": """INSERT INTO xwrag_DOC_FULL (id, content, workspace)
                        VALUES ($1, $2, $3)
                        ON CONFLICT (workspace,id) DO UPDATE
                           SET content = $2, update_time = CURRENT_TIMESTAMP
                       """,
    "upsert_llm_response_cache": """INSERT INTO xwrag_LLM_CACHE(workspace,id,original_prompt,return_value,chunk_id,cache_type,queryparam)
                                      ...
                                      update_time = CURRENT_TIMESTAMP
                                     """,
    "upsert_text_chunk": """INSERT INTO xwrag_DOC_CHUNKS (workspace, id, tokens,
                     ...
                     """,
    "upsert_full_entities": """INSERT INTO xwrag_FULL_ENTITIES (workspace, id, entity_names, count,
                      create_time, update_time)
                      VALUES ($1, $2, $3, $4, $5, $6)
                      ON CONFLICT (workspace,id) DO UPDATE
                      ...
                      update_time = EXCLUDED.update_time
                     """,
    "upsert_full_relations": """INSERT INTO xwrag_FULL_RELATIONS (workspace, id, relation_pairs, count,
                      create_time, update_time)
                      VALUES ($1, $2, $3, $4, $5, $6)
                      ON CONFLICT (workspace,id) DO UPDATE
    ...
    "upsert_chunk": """INSERT INTO xwrag_VDB_CHUNKS (workspace, id, tokens,
                      ...
                      update_time = EXCLUDED.update_time
                     """,
    "upsert_entity": """INSERT INTO xwrag_VDB_ENTITY (workspace, id, entity_name, content,
                      ...
                      update_time=EXCLUDED.update_time
                     """,
    "upsert_relationship": """INSERT INTO xwrag_VDB_RELATION (workspace, id, source_id,
