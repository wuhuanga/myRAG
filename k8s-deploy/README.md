...
This is the Helm chart for xwrag, used to deploy xwrag services on a Kubernetes cluster.

There are two recommended deployment methods for xwrag:
...
* **Helm** (v3.x+)
  * Kubernetes package manager used to install xwrag.
...
You can deploy xwrag using either the provided convenience script or direct Helm commands. Both methods configure the same environment variables defined in the `xwrag/values.yaml` file.
...
helm upgrade --install xwrag ./xwrag \
  --namespace rag \
...
xwrag and its databases are deployed within the same Kubernetes cluster, making configuration straightforward.
...
You can configure xwrag's resource usage by modifying the `values.yaml` file:
...
The `env` section in the `values.yaml` file contains all environment configurations for xwrag, similar to a `.env` file. When using helm upgrade or helm install commands, you can override these with the --set flag.
...
env:
  ...
  xwrag_KV_STORAGE: PGKVStorage              # Key-value storage type
  xwrag_VECTOR_STORAGE: PGVectorStorage      # Vector storage type
  xwrag_GRAPH_STORAGE: Neo4JStorage          # Graph storage type
  xwrag_DOC_STATUS_STORAGE: PGDocStatusStorage  # Document status storage type
```
...
- For more customized configurations, please refer to the official xwrag documentation
